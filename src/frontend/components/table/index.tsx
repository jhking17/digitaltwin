...
export interface FinalSimpleTableProps extends simpleTableProps {
    /** iModel whose contents should be displayed in the table */
}

/** Table component for the viewer app */
export const SimpleTableComponent: React.FunctionComponent<FinalSimpleTableProps> = props => {
    // eslint-disable-line @typescript-eslint/naming-convention
    const dataProvider = useDisposable(
        useCallback(
            () =>
                new PresentationTableDataProvider({ imodel: props.imodel, ruleset: RULESET_TABLE }),
            [props.imodel]
        )
    );
    return <SimpleTable dataProvider={dataProvider} />;
};
